...
{
	m_pKeyMapSdl = SDL_GetKeyboardState(&m_KeyboardLength);//Set this map to be constantly updated
...
{
	std::memcpy(m_pKeyMapOld, m_pKeyMapNew, m_KeyboardLength);//Update Old Keyboard state
	...
	std::memcpy(m_pKeyMapNew, m_pKeyMapSdl, m_KeyboardLength);//Update New Keyboard state
